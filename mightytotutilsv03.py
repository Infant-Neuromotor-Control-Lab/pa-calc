...
# ****************************************************************************
# File I/O options
fileNameNoExtension = "DUMMY_FILE_NAME"  # same 3 day above
...
def displayPrintBuffer():
    ...
    print(displaytexts.printBufferStr)
...
def writeErrorLogToFile(outputDirStr, fNameStr, fNameAppendStr, fExtensionStr):
    ...
    writeSummaryStatsToFile(displaytexts.printBufferStr, outputDirStr, fNameStr, fNameAppendStr, fExtensionStr) # this function just writes a string to file
    ...
    try:
        ...
        if(fExtension == ".hdf5"):
            sensorAccelUnit, sensorSamplingRate, sensorTime, sensorAccelXYZ, sensorGyroXYZ, sensorTemp = loadDataFromPIDFFile(
                fNameInputDir + '/' + fNameNoExtension + fExtension)
        elif(fExtension == ".tsv"):
        ...
        printToPrintBuffer("Error: Problem reading input file")
...
# @return Void
#
def processDataSaveResults(fileReadSensorTime, fileReadSensorAccelXYZ, minFileTime, maxFileTime):
    ...
    if(fileReadErrorFlag == 0):
        ...
        if(dataInputErrorFlag == 0):
            ...
            accelObjectSelectedRegion = computePhysicalActivity(computedQttyOption, optimizationTypeOption, accelObjectSelectedRegion, accelMagBaseline, infantLegLengthCm, constRefLegLength3To5MonthCm, windowLen, deltaTimeSec)
            ...
            writeSummaryStatsToFile(summaryStatsString, outputDir, fileNameNoExtension, fileNameAppendSummary, outputFileExtension)
            ...
            writeActivityBoutsToFile(groupedActivityRegionsObject, outputDir, fileNameNoExtension, fileNameAppendBouts, outputFileExtension)
            
            # Output 3, raw time/activity classification
            printToPrintBuffer("INFO: Writing all activity labels to file")
            writeActivityAllToFile(accelObjectSelectedRegion, outputDir, fileNameNoExtension, fileNameAppendRaw, outputFileExtension) # accelObject PhysActSeries with index, time, accelMagLeft, accelMagRight filled in (m/s2)
    ...
    writeErrorLogToFile(outputDir, fileNameNoExtension, fileNameAppendErrorLog, outputFileExtension)
    printToPrintBuffer("INFO: Done processing and saving")
...
def mainFunction():
    ...
    fileReadSensorTime, fileReadSensorAccelXYZ, minInputFileTime, maxInputFileTime = loadFileDisplayFile(inputDir, fileNameNoExtension, inputFileExtension)

    #  STEP 2: PROCESS FILE, SAVE RESULTS    
    processDataSaveResults(fileReadSensorTime, fileReadSensorAccelXYZ, minInputFileTime, maxInputFileTime)
